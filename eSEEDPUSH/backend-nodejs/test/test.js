...
describe('User tests: ', () => {
  before ( function(done) {
    ...
    salmaTag.save((err, tag) => {
      ...
      salmaNonExpert = new Users({ username: "Salma1" , email: "salma1@gmail.com" , password: "10" , 
      role: "user" , speciality : [tag._id] });
      done();
    });
  })
  ...
  it("Searching for a certain tag.", function(done){
    ...
    it("Add an expert to bookmarks.", function(done){
   
      chai.request(app)
          .post('/api/user/addToBookmarks/'+ salmaExpert._id)
          .set('authorization', token)
          .end(function(err,res) {
              res.should.have.status(201);
              res.body.should.have.property("msg");
              res.body.msg.should.be.equal("The expert was successfully added to your array of bookmarks.");        
              res.body.should.have.property("data");
              //res.body.data.should.be.eql(salmaExpert._id);
             // res.body.data.should.have.property(salma).eql(salmaExpert._id);
              done();
          })
      });
      it("Shoud not expert to bookmarks as it is already there.", function(done){
           ...
           chai.request(app)
               ...
               .end(function(err,res) {
                   ...
                   res.body.should.have.property("msg");
                   res.body.msg.should.be.equal("The expert could not be added to your "
                   +"bookmarks because  he/she is already added to your bookmarks.");        
                   //res.body.should.have.property("data");
                   done();
               })
           });    
         /*  it("Shoud not add non expert to bookmarks.", function(done){
            
               chai.request(app)
                   .post('/api/user/addToBookmarks/'+ salmaNonExpert._id)
                   .set('authorization', token)
                   .end(function(err,res) {
                       res.should.have.status(404);
                       res.body.should.have.property("msg");
                       res.body.msg.should.be.equal("The expert you are trying to add to your bookmarks is "+
                       "either not found or not an expert.");        
                       //res.body.should.have.property("data");
                       done();
                   })
               });   */ 
  /*  
      it("View experts bookmarked.", function(done){
            
               chai.request(app)
                   .get('/api/user/viewBookmarks/')
                   .set('authorization', token)
                   .end(function(err,res) {
                       res.should.have.status(200);
                       res.body.should.have.property("msg");
                       res.body.msg.should.be.equal("Bookmarks retrieved successfully.");        
                       res.body.should.have.property("data");
                       //res.body.data.should.be.eql(salmaExpert._id);
                      // res.body.data.should.have.property(salma).eql(salmaExpert._id);
                       done();
      ...
      it("Remove an expert from bookmarks", function(done){
                
              chai.request(app)
                       .delete('/api/user/removeFromBookmarks/'+ salmaExpert._id)
                       .set('authorization', token)
                       .end(function(err,res) {
                           res.should.have.status(201);
                           res.body.should.have.property("msg");
                           res.body.msg.should.be.equal("bookmark removed");        
                           res.body.should.have.property("data");
                           //res.body.data.should.be.eql(salmaExpert._id);
                          // res.body.data.should.have.property(salma).eql(salmaExpert._id);
                           done();
  ...
  it("Request to reserve slot.", function(done){
                
              chai.request(app)
                       .post('/api/user/chooseSlot/'+ "e@gmail.com")
                       .send({ sessionId: '5ae8e47b3e74a7198c5eeda3' })
                       .set('authorization', token)
                       .end(function(err,res) {
                           res.should.have.status(200);
                           res.body.should.have.property("msg");
                           res.body.msg.should.be.equal("Successfully requested to reserve this slot.");        
                           res.body.should.have.property("data");
                           //console.log(res.body);
                           //res.body.data.should.be.eql(salmaExpert._id);
                           res.body.data.should.have.property("expertEmail").eql("e@gmail.com");
                           done();
                       })
                   });
